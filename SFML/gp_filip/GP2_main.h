...
double calculateFitness(Tree& calcTree, int mapNum) {
    ...
    for (auto& map : Parameters::maps) {
        ...
        Bird bird;
        Controller* controller = new GP2Controller(calcTree.root);
...
Controller* GP_FC(sf::RenderWindow& window) {

    Controller* sol = nullptr;
    ...
    else if (option == '1') sol = simulatePopulation();
    return sol;
}
