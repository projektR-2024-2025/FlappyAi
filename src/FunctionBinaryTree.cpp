...
Node* FunctionBinaryTree::createRandomNode(int max_dubina, int dubina) {
        int type;
        string value ;
...
void FunctionBinaryTree::printTree(Node* node) {
...
float FunctionBinaryTree::izracunajR(Node *node, vector<float> ulaz){
...
string FunctionBinaryTree::toString(Node* node) {
    if (!node) return "";
...
int FunctionBinaryTree::countNodes(Node* node) { 
...
bool FunctionBinaryTree::areNodesEqual(Node* node1, Node* node2){
...
Node* Node::copy()  {
        Node* newNode = new Node(value, type); // Create a new node with the current node's value and type
        if (left) {
